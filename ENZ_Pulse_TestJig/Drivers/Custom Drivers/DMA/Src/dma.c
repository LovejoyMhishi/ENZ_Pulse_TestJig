...
void DMA1_Init(void) {
	...
	SET_BIT(DMA1_Channel1->CCR, DMA_CCR_CIRC);                  //0: Circular mode disabled
...
void ADC_Stop_DMA(ADC_TypeDef *ADCx) {
	...
	CLEAR_BIT(ADCx->CR, ADC_CR_ADSTART);                           //Stopping conversions (ADSTART)
    while (READ_BIT(ADCx->CR, ADC_CR_ADSTART)) {}
